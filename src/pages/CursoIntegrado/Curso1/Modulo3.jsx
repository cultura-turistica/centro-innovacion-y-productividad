              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', marginTop: '2rem' }}>
                ...
                <div style={{ flex: '1 1 300px', background: 'white', borderRadius: '25px', overflow: 'hidden', boxShadow: '0 20px 40px rgba(0,0,0,0.08)', display: 'flex', flexDirection: 'column', border: '1px solid #e2e8f0' }}>
                  <div style={{ height: '220px', position: 'relative', background: '#dbeafe', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <div style={{ position: 'absolute', top: '15px', right: '15px', background: '#032968', color: 'white', padding: '5px 15px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 'bold' }}>{data.casos.tangible.tag}</div>
                  </div>
                  <div style={{ padding: '2rem' }}>
                    ...
                    <h4 style={{ fontSize: '1.5rem', color: '#0f172a', fontWeight: 800, margin: '0 0 15px 0' }}>{data.casos.tangible.title}</h4>
                    <p style={{ color: '#475569', fontSize: '1.05rem', lineHeight: '1.6', margin: 0 }}>
                      <strong>Antes del turismo:</strong> {data.casos.tangible.before}<br /><br />
                      <strong>Impacto SPEC:</strong> <span dangerouslySetInnerHTML={{ __html: data.casos.tangible.impact }}></span>
                    </p>
                  </div>
                </div>
                ...
                <div style={{ flex: '1 1 300px', background: 'white', borderRadius: '25px', overflow: 'hidden', boxShadow: '0 20px 40px rgba(0,0,0,0.08)', display: 'flex', flexDirection: 'column', border: '1px solid #e2e8f0' }}>
                  <div style={{ height: '220px', position: 'relative', background: '#ffedd5', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <div style={{ position: 'absolute', top: '15px', right: '15px', background: '#ea580c', color: 'white', padding: '5px 15px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 'bold' }}>{data.casos.intangible.tag}</div>
                  </div>
                  <div style={{ padding: '2rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#64748b', fontSize: '0.9rem', fontWeight: 600, marginBottom: '10px' }}>
                      <Map size={16} /> {data.casos.intangible.location}
                    </div>
                    <h4 style={{ fontSize: '1.5rem', color: '#0f172a', fontWeight: 800, margin: '0 0 15px 0' }}>{data.casos.intangible.title}</h4>
                    <p style={{ color: '#475569', fontSize: '1.05rem', lineHeight: '1.6', margin: 0 }}>
                      <strong>Antes del turismo:</strong> {data.casos.intangible.before}<br /><br />
                      <strong>Impacto SPEC:</strong> <span dangerouslySetInnerHTML={{ __html: data.casos.intangible.impact }}></span>
                    </p>
                  </div>
                </div>
              </div>
