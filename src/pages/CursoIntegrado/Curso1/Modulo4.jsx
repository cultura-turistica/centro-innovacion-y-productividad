    ...
    <div className="fade-in" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: '2rem clamp(1rem, 3vw, 3rem)' }}>
              ...
              <div style={{ background: '#f1f5f9', borderRadius: '25px', padding: '3rem 2rem', textAlign: 'center', border: '2px dashed #cbd5e1' }}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: '#fef3c7', color: '#d97706', padding: '8px 20px', borderRadius: '30px', fontWeight: 800, fontSize: '0.9rem', textTransform: 'uppercase', marginBottom: '1rem' }}><Edit3 size={16} /> {data.dinamica1.label}</div>
                <h3 style={{ color: '#0f172a', margin: '0 0 10px 0', fontSize: '1.8rem', fontWeight: 900 }}>{data.dinamica1.title}</h3>
                <p style={{ color: '#475569', fontSize: '1.1rem', marginBottom: '3rem' }}>{data.dinamica1.description}</p>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
                  {[
                    { id: 'alianzas', label: data.dinamica1.zones[0].label, icon: <HeartHandshake size={30} color="#3b82f6" /> },
                    { id: 'dinero', label: data.dinamica1.zones[1].label, icon: <TrendingUp size={30} color="#10b981" /> },
                    { id: 'turistas', label: data.dinamica1.zones[2].label, icon: <Map size={30} color="#f59e0b" /> },
                    { id: 'redes', label: data.dinamica1.zones[3].label, icon: <Camera size={30} color="#8b5cf6" /> }
                  ].map(zone => (
                    <div
                ...
                {Object.keys(matchedRoles).length === 4 && (
                  <div className="pop-in" style={{ marginTop: '3rem', background: '#e0e7ff', padding: '3rem 2rem', borderRadius: '25px', textAlign: 'center' }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: '#c7d2fe', color: '#4338ca', padding: '8px 20px', borderRadius: '30px', fontWeight: 800, fontSize: '0.9rem', textTransform: 'uppercase', marginBottom: '1rem' }}><Edit3 size={16} /> {data.dinamica2.label}</div>
                    <h3 style={{ color: '#312e81', margin: '0 0 10px 0', fontSize: '1.8rem', fontWeight: 900 }}>{data.dinamica2.title}</h3>
                    <p style={{ color: '#4338ca', fontSize: '1.1rem', marginBottom: '3rem' }}>{data.dinamica2.description}</p>
                    ...
                    <div style={{ width: '300px', height: '300px', position: 'relative', margin: '2rem auto' }}>
                      ...
                      {[{ id: 1, top: 0, left: 0, label: data.dinamica2.nodes[0].label }, { id: 2, top: 0, right: 0, label: data.dinamica2.nodes[1].label }, { id: 3, bottom: 0, right: 0, label: data.dinamica2.nodes[2].label }, { id: 4, bottom: 0, left: 0, label: data.dinamica2.nodes[3].label }].map(node => (
                    ...
                    {activeNodes.length === 4 && (
                      <div className="pop-in" style={{ background: 'white', padding: '1.5rem', borderRadius: '15px', maxWidth: '500px', margin: '0 auto', textAlign: 'center' }}>
                        <h4 style={{ color: '#312e81', margin: '0 0 10px 0', fontSize: '1.2rem' }}>{data.dinamica2.successMsg.title}</h4>
                        <p style={{ color: '#475569', margin: 0 }}>{data.dinamica2.successMsg.description}</p>
                      </div>
                    )}
                  </div>
                )}
