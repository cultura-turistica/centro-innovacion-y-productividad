export const certificacionData = {
  badge: "Cierre y Evaluación",
  title: "Certificación<br />Oficial",
  description: "Demuestra lo aprendido en el curso y obtén tu sello verificable de Cultura T.",
  completionTitle: "¡Has completado el contenido!",
  completionDesc: "Felicidades por completar el estudio de los módulos de Fotografía y Composición. Si lo deseas, puedes realizar la evaluación de conocimientos para obtener tu certificado oficial.",
  exitBtn: "Volver a Inicio",
  certBtn: "Sí, Quiero Certificarme",
  formularioLegal: {
    title: "Paso 3: Formulario de Registro",
    description: "Por favor, ingresa tus datos legales tal como deseas que aparezcan en tu certificado oficial.",
    privacyNoticeTitle: "Aviso de Privacidad:",
    privacyNotice: "Tus datos serán vinculados al Sello Matemático criptográfico y registrados en el libro oficial de egresados de Cultura T para efectos de verificación pública de tus competencias.",
    nameLabel: "Nombres y Apellidos Completos",
    namePlaceholder: "Ej. Juan Pérez López",
    idLabel: "Número de Identificación (C.C. o NIT)",
    idPlaceholder: "Ej. 1.020.304.050",
    processingBtn: "Generando...",
    continueBtn: "Generar Certificado Oficial"
  },
  encuesta: {
    title: "Paso 1: Encuesta de Satisfacción",
    description: "Para continuar a la calificación y certificar tu participación, debes ayudarnos a mejorar respondiendo esta encuesta.",
    anonymousNotice: "(Tus respuestas aquí son 100% anónimas)",
    questions: [
      {
        id: "q1",
        label: "1. ¿Consideras que este curso te ha permitido adquirir nuevos conocimientos que antes no tenías?",
        options: ["Mucho", "Algo", "Poco"]
      },
      {
        id: "q2",
        label: "2. ¿Qué tanto crees que ha mejorado tu comprensión sobre el tema tratado?",
        options: ["Notablemente", "Moderadamente", "Nada"]
      },
      {
        id: "q3",
        label: "3. ¿Sientes que lo aprendido te será verdaderamente útil para aplicarlo en tu entorno profesional o proyecto?",
        options: ["Totalmente", "Parcialmente", "No"]
      },
      {
        id: "q4",
        label: "4. ¿El formato y contenido del curso facilitaron tu proceso de aprendizaje?",
        options: ["Sí, muy claro", "Regular", "No fue claro"]
      },
      {
        id: "q5",
        label: "5. ¿Consideras que tus habilidades o capacidades se han fortalecido gracias a esta capacitación?",
        options: ["Definitivamente", "Tal vez", "No"]
      }
    ],
    commentsLabel: "6. (Opcional) Déjanos tus comentarios o sugerencias sobre la calidad del curso.",
    submitBtn: "Enviar y Pasar a la Evaluación"
  },
  quiz: {
    title: "Paso 2: Evaluación de Conocimientos",
    courseName: "Curso Práctico de Fundamentos y Composición Fotográfica",
    courseId: "FOTO-1",
    horas: "20",
    questions: [
      {
        id: "q1",
        text: "¿Cuál de estos ajustes controla directamente la cantidad de ruido digital (grano) que tendrá la fotografía?",
        options: [
          { id: "a", text: "Apertura del Diafragma" },
          { id: "b", text: "Velocidad de Obturación" },
          { id: "c", text: "Sensibilidad ISO" }
        ],
        // 'c' hash
        correctHash: "2e7d2c03a9507ae265ecf5b5356885a53393a2029d241394997265a1a25aefc6"
      },
      {
        id: "q2",
        text: "Si deseas fotografiar un sujeto en movimiento (como un colibrí o un auto) y quieres congelarlo en el aire, ¿qué debes hacer?",
        options: [
          { id: "a", text: "Usar una velocidad de obturación rápida (ej. 1/1000s)." },
          { id: "b", text: "Usar un ISO muy bajo (ej. 100)." },
          { id: "c", text: "Usar una apertura de f/22." }
        ],
        // 'a' hash
        correctHash: "ca978112ca1bbdcafac231b39a23dc4da786eff8147c4e72b9807785afee48bb"
      },
      {
        id: "q3",
        text: "¿Qué efecto visual consigues al utilizar una apertura de diafragma muy grande (ej. f/1.8)?",
        options: [
          { id: "a", text: "Que todo el paisaje quede perfectamente nítido y enfocado." },
          { id: "b", text: "Que el fondo detrás de tu sujeto quede borroso (poca profundidad de campo o Bokeh)." },
          { id: "c", text: "Que la foto quede con un filtro azulado." }
        ],
        // 'b' hash
        correctHash: "3e23e8160039594a33894f6564e1b1348bbd7a0088d42c4acb73eeaed59c009d"
      },
      {
        id: "q4",
        text: "En composición, ¿qué dicta la 'Regla de los Tercios'?",
        options: [
          { id: "a", text: "Que debes situar el elemento principal exactamente en el centro de la imagen." },
          { id: "b", text: "Que debes usar tres colores diferentes en tu fotografía." },
          { id: "c", text: "Que debes ubicar los elementos importantes en las intersecciones de una cuadrícula de 3x3." }
        ],
        // 'c' hash
        correctHash: "2e7d2c03a9507ae265ecf5b5356885a53393a2029d241394997265a1a25aefc6"
      },
      {
        id: "q5",
        text: "¿Qué parámetro usarías para ajustar la temperatura de color si la fotografía se ve muy 'naranja' porque estás bajo luces de tungsteno?",
        options: [
          { id: "a", text: "El Balance de Blancos (White Balance)." },
          { id: "b", text: "La Velocidad de Obturación." },
          { id: "c", text: "El número f/." }
        ],
        // 'a' hash
        correctHash: "ca978112ca1bbdcafac231b39a23dc4da786eff8147c4e72b9807785afee48bb"
      }
    ]
  }
};
